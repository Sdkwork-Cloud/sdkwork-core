...
const genericAppSdkReferenceFiles = [
  path.join(workspaceRoot, "package.json"),
  path.join(workspaceRoot, "pnpm-workspace.yaml"),
  packageJsonPath,
  path.join(packageRoot, "vite.config.ts"),
...
const genericAppSdkTokens = [
  genericAppSdkPackage,
  genericAppSdkPath,
  legacySpringAppApiPath,
];
const genericAppSdkOffenders = genericAppSdkReferenceFiles.flatMap((filePath) => {
  const contents = readFileSync(filePath, "utf8");
  return genericAppSdkTokens.flatMap((token) =>
    contents.includes(token) ? [`${path.relative(workspaceRoot, filePath)}: ${token}`] : [],
  );
});

assert.deepEqual(
  genericAppSdkOffenders,
  [],
  `Desktop core must not depend on the retired generic app SDK.\n${genericAppSdkOffenders.join("\n")}`,
);
